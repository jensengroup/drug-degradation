...
import numpy as np
from rxnnet.chemutils import hartree2kcalmol, read_multi_xyz, xyz2ac
...
End"""
    ...
    if normal_termination(lines) or force:
        ...
        if any(("opt" in key) or ("ts" in key) for key in clean_option_keys):
...
def read_vibrations(lines: List[str]) -> list:
    ...
    for i in range(int(np.ceil(n_vibs / 6))):
        tmp_modes = []
        for line in lines[
            mode_start_idx + 8 + i * (n_vibs + 1) : mode_start_idx
            + 8
            + (i + 1) * n_vibs
            + i
        ]:
...
def get_detailed_contributions(lines: List[str]) -> dict:
    for i, line in enumerate(lines):
        if "Zero point energy" in line:
            zero_point_energy = float(line.split()[-4])
        elif "Thermal vibrational correction" in line:
            thermal_vibrational_correction = float(line.split()[-4])
        elif "Thermal rotational correction" in line:
            thermal_rotational_correction = float(line.split()[-4])
        elif "Thermal translational correction" in line:
            thermal_translational_correction = float(line.split()[-4])
        elif "Thermal Enthalpy correction" in line:
            thermal_enthalpy_correction = float(line.split()[-4])
        elif "Electronic entropy" in line:
            electronic_entropy = float(line.split()[-4])
        elif "Vibrational entropy               ..." in line:
            vibrational_entropy = float(line.split()[-4])
        elif "Rotational entropy                ..." in line:
            rotational_entropy = float(line.split()[-4])
        elif "Translational entropy             ..." in line:
            translational_entropy = float(line.split()[-4])
        elif "G-E(el)" in line:
            gibbs_correction = float(line.split()[-4])
